...
const MoviesAndTVShows = ({ personId }) => {
 ...
 <h1 className='lg:text-7xl md:text-4xl text-3xl font-semibold text-blue'> Known For</h1>
      ...
      <div className=" lg:text-3xl text-2xl py-10">
        <button
          className={`mr-4 focus:outline-none ${mediaType === 'movie' ? 'text-blue-400 border-[1px] rounded-xl  p-3 border-blue' : 'text-gray-400'}`}
          onClick={() => handleToggle('movie')}
        >
          Movies
        </button>
        <button
          className={`focus:outline-none ${mediaType === 'tv' ? 'text-blue-400 border-[1px] rounded-xl  p-3 border-blue' : 'text-gray-400'}`}
