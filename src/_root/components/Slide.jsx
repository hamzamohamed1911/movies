  ...
  return (
    <div className="relative">
      ...
      <div className="absolute inset-0 bg-black bg-opacity-50 rounded-tl-[50px]">
        ...
        >
          <h2 className="text-babyblue font-semibold lg:text-5xl text-3xl">{title || name}</h2>

          <div className="flex items-center gap-2">
            <span className="lg:text-2xl md:text-xl sm:text-lg text-babyblue">{vote_average}</span>
            {[...Array(5)].map((_, i) => (
              <AiFillStar
                key={i}
                color={i < Math.round(vote_average / 2) ? 'gold' : 'grey'}
                className="lg:h-8 lg:w-8 h-7 w-7"
              />
            ))}
          </div>
