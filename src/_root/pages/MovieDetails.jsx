...
const MovieDetails = () => {
  ...
  return (<>
  <section className='bg-gradient-to-b from-transparent to-black h-full lg:p-20 p-10 '>
  <Details item={item} />;
  <div className='lg:p-28'>
  <div className="lg:max-w-4xl max-w-xl lg:px-4">
...
<div className="w-[500px] py-14  ">
    ...
    <div className="w-[500px] py-14  ">
      <h1 className="text-white text-3xl mb-8">SIMILLAR</h1>
      <Slider {...settings}>
        {slides.map((slide, index) => (
          <div key={slide.id}>
            <img
...
<div className=' py-36 container'>
  <div className='lg:w-[1200px] w-screen '>
        ...
        {slides.map((slide, index) => (
          <div key={slide.id}>
            <img
              src={slide.posterUrl}
              alt={slide.title}
              className="cursor-pointer rounded-lg w-80 h-96 mx-auto"
            />
            <h1 className="text-babyblue text-2xl text-center">{slide.title}</h1>
          </div>
        ))}
